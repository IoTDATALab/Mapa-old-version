...
client.subscribe([("soa_init_params", 2), ("soa_params/" + CLIENT_ID, 2)])
client.loop_start()
...
if __name__=='__main__':
    ...
    for epoch in range(EPOCH):
        for step, (b_x, b_y) in enumerate(train_loader):  
            ...
            loss.backward()    
            if step % TEST_NUM == 0:
                man_file = open(RESULT_ROOT+'[EDGE_NUM:'+str(DELAY)+']'+'[METHOD:soa]', 'w')
            ...
            client.publish("soa_grads/" + CLIENT_ID, cPickle.dumps(grads), 2)
            params = cPickle.loads(msgQueue.get())
            for i,f in enumerate(cnn.parameters()):
                f.data = params[i].float()